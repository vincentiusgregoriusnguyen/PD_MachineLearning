...
def main():
    ...
    MLA =  KNeighborsClassifier(n_jobs = -1)
    lb = preprocessing.LabelBinarizer()
    lb.fit(y)


    sfs1 = SFS(estimator=MLA, k_features=(1, n),forward=True,  floating=False,  scoring='roc_auc',cv=StratifiedKFold(10))	
    pipe = make_pipeline(StandardScaler(), sfs1)
    k = pipe.fit_transform(X, y);
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        tempMLA = KNeighborsClassifier(n_jobs = -1);
